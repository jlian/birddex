...
# (`boundary=administrative`, admin_level 2-4), so forward and reverse search
# cannot resolve a coordinate to different jurisdictions.
set -euo pipefail
...
# belong to a boundary. Measured globally: 288,613 features, of which 5,003
# carry a code. Filtering here takes the enrichment input from 4.2 GB to 1.1 GB.
...
with open(src, encoding="utf-8") as fh, open(dst, "w", encoding="utf-8") as out:
    for line in fh:
        ...
        out.write("\n")
        kept += 1
print(f"  boundaries carrying an ISO code: {kept:,}", file=sys.stderr)
if kept == 0:
    sys.exit("no ISO-coded boundaries found; refusing to write an empty admin file")
PY

rm -f "$RAW"
echo "TOTAL $(wc -l < "$OUT/admin-iso.geojsonseq") ISO boundaries, $(du -h "$OUT/admin-iso.geojsonseq" | cut -f1)" | tee -a "$LOG"
