...
| G14 | Fold the text classifier into the ONNX graph | The classifier is a constant 11167x768 matrix, and the client currently multiplies it in scalar JS, which is 8.6M multiply-adds, single threaded, no SIMD. Cost on a phone is UNMEASURED | ⬜ | **Not started, and gated on measurement.** The matmul finds the top 25 candidates, and the Bayesian rerank after it is trivial, at 25 candidates and microseconds. So retrieval is the expensive half, not the scoring. Folding it in means appending a `MatMul` node with the matrix as an initialiser, so the graph becomes image to 11167 similarities and onnxruntime runs it on WebGPU or WASM SIMD instead of scalar JS. Output shape goes 768 to 11167 and `text_classifier_fp16.bin` disappears. ⚠️ Adds about 16 MiB to the weights, which pushes the 24.00 MiB `.data` past the 25 MiB cap, so it forces the sharded path from [G12](#phase-g-ship) or a second external file. **Measure the scalar loop on a real phone FIRST.** |
| G15 | Decide where the Bayesian rerank runs | The rerank is easy math on 25 candidates. Running it server-side lets calibration change without a client redownload | ⬜ | **Open, leaning client-side.** The embedding is 768 floats, about 3 KB, so the wire cost is far below the photo upload it replaces, and the server cost is trivial next to the gpt-5.4-mini call it removes. Against: it **breaks offline**, which was the point of going client-side, and it turns a graceful degradation into a hard dependency, because a client holding an embedding with no classifier cannot name a species at all. Today a missing prior cell falls back to vision-only. Note the split does not land where the volatility is: calibration is about 400 bytes and the prior is 5.41 MiB with an ETag, so serving BOTH with cache headers buys most of the agility while keeping inference local. Revisit only if mobile measurement in G14 forces it. |
| G16 | Add month to the occurrence prior | The prior is P(species\|cell) with no time dimension, so a Rufous Hummingbird in Seattle gets the same score in July and January | ⬜ | **The data exists and we are discarding it.** `observed_on` is a column in `ml/inat-metadata/observations.csv.gz` sitting next to the lat/lon already used, and `build_occurrence.py` selects only `taxon_id, latitude, longitude`. Fixtures already carry `month` in context and the scorer ignores it. **This is a capability the Bayesian rewrite LOST**: the old BirdLife path had `seasonalTrust` with breeding, nonbreeding and passage masks, so migratory species used to get a seasonal signal and now do not. Build is the same 2-minute DuckDB pass with `month(observed_on)` added to the GROUP BY. ⚠️ **Sparsity is the real risk**: 3,176,965 species-cell pairs split 12 ways leaves most cell-months with very thin counts, so a naive swap replaces a solid estimate with noise. It needs smoothing toward the month-agnostic prior, which is what the unused `alpha` pseudo-count in `fit_occurrence.py` is for. Gate: it must beat 93.80 on the same split. |
| G13 | Export the shipping student to ONNX | int8 was chosen on the PyTorch artifact in [F10](#phase-f-shrink-the-model-to-clear-the-size-gate). The export itself has to be proven neutral before any of that transfers | ✅ | `wise_a0.60.pt` from `runs/ft_tiny39_fresh` exports clean: worst cosine(pytorch, onnx) **1.00000000**, worst absolute difference **7.34e-07**, **0/16** top-1 disagreements. The script exits non-zero on mismatch, so it cannot pass silently. Artifacts: `onnx_tiny39/wingclip_visual_fp32.onnx` and `text_classifier.npy` at (11167, 768). Parameter counts reconcile: 38.33M visual plus 0.39M projection is 38.72M, and the 38.3M quoted elsewhere is the backbone only. **fp16 export is broken** and excluded: `convert_float_to_float16` emits Cast nodes whose declared output type does not match the consumer, so onnxruntime rejects the graph at load. fp16 is a Core ML and WebGPU format, so this does not block the web ship path. |
