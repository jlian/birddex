...
| G20 | Fit the absent-species floor instead of hardcoding it | A candidate species with no record in the cell cannot have a probability looked up, so the scorer substitutes a constant. That constant was never fitted, and the reference and the client disagree on it | ⬜ | **Found while reviewing G16. The two implementations differ.** `rank.ts` uses `OCC_FLOOR = log(1e-9)`, which is **-20.7**, and at beta 0.563 that is a -11.7 hit to the score. `fit_occurrence.py` never uses that value: for an absent species it evaluates `log(0 + alpha*pri) - log(den)` with a fitted alpha near 3e-10, then clamps at 1e-12, so its effective floor is about **-27.6**. So `T` and `beta` were fitted against a harsher absence penalty than the shipping client applies. Both land at 93.6 to 93.8, so the disagreement is not currently costing much, but it is an unfitted free parameter that differs between the reference and the artifact. Make the floor a fitted parameter in the same LBFGS pass that already optimises `logT`, `wfree`, `logalpha` and `logbeta`, then export it into the calibration JSON so the client reads it rather than hardcoding. This matters more for [G16](#phase-g-ship): a monthly prior makes absence far more common, so the floor stops being a rare correction and becomes a main term. |
| G16 | Add month to the occurrence prior | The prior is P(species\|cell) with no time dimension, so a Rufous Hummingbird in Seattle gets the same score in July and January | ⬜ | **Data exists, but a naive monthly split WILL lose accuracy. Measured.** `observed_on` is already a column in the iNat dump next to the lat/lon we use, and `build_occurrence.py` selects only `taxon_id, latitude, longitude`. This is a capability the Bayesian rewrite LOST: the old BirdLife path had `seasonalTrust` with breeding, nonbreeding and passage masks. ⚠️ **The corpus is big but the per-pair counts are not.** Over 26,396,703 (species, cell) pairs and 157,114,210 observations: median count is **1**, p90 is 10, **50.1% of pairs have exactly one observation**, and 91.3% have fewer than 12. Only **8.7%** reach even one observation per month on average. Those thin pairs still hold 35.7% of all observations, so this is not an edge case, and it hits RARE species hardest, which are the ones a geographic prior exists to rank correctly. The specific hazard is that the scorer treats a missing species as `OCC_FLOOR = log(1e-9)`, strong evidence of absence. That is fair annually and wrong monthly, where a zero usually means nobody birded that cell that month. So the build needs hierarchical backoff from (species, cell, month) toward (species, cell) when counts are thin, rather than a per-month swap. The `alpha` pseudo-count in `fit_occurrence.py` is the right hook and currently fits to about 1e-10 because it has nothing to smooth toward. Gate: beat 93.80 on the same split. |
