...
| D7 | Fine-tune the student on photos the TEACHER never saw | `ft_tiny39` uses the same `groundtruth_heldout_distilled.parquet` that `ft_clean_01` used to build WingCLIP-0.1, so the student relearns supervised signal its own teacher already holds | ❓ | This does NOT invalidate any NABirds number, because NABirds is a separate dataset. It does make "the student approaches its teacher" partly circular, and it likely UNDERSTATES what a clean fine-tune gains. Fresh photos exist: `build_manifest.py` capped each species by lowest `photo_id`, so about 49M photos are untouched. Pass BOTH `--train-manifest` and `--exclude-manifest groundtruth_heldout_distilled.parquet` to [`build_groundtruth_split.py`](build_groundtruth_split.py) for a third disjoint set. Cost is a DuckDB scan over the 30 GB metadata dump, then a ~150k image download. |
