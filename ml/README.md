...
| D7 | Fine-tune the student on photos the TEACHER never saw | `ft_tiny39` used the same ground-truth set that produced WingCLIP-0.1, so the student relearned supervised signal that its own teacher already held | ✅ | **Confirmed, and it cost real accuracy.** A disjoint set of 143,890 photos over 3,662 species, excluded by photo_id AND observation_uuid from both the distillation corpus and the phase-2 set, beat the reused set at EVERY alpha by +0.54 to +0.84. Same baseline, same recipe, only the photos differ. The fine-tune gain rose from +1.10 to **+1.92** (84.98 to 86.90). Reusing the teacher's own fine-tune data suppresses the student gain by roughly 40%. Build a disjoint set with `--exclude-manifest`. About 46.1M untouched candidate photos remain. |
...
| F8 | Fine-tune and sweep alpha | Same chain as [D3 and D4](#phase-d-beat-the-teacher) applied to the new student | ✅ | **SHIP BAR CLEARED: 86.90 against BioCLIP-2 at 86.41.** Best alpha is 0.60, with 0.75 tied at 86.90, on the full 24,633-image eval. The optimum moved DOWN from the 0.90 that ViT-B wanted, as predicted for a 2.26x smaller model. Full sweep: 0.25 86.27, 0.40 86.64, 0.50 86.82, 0.60 **86.90**, 0.75 86.90, 0.90 86.56. This used the FRESH photo set, see [D7](#phase-d-beat-the-teacher). The reused set peaked at only 86.08. Retention against the WingCLIP-0.1 teacher (89.93) is 96.6% at 2.26x fewer parameters. |
