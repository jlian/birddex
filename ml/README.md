...
| G17 | Measure quantisation on the TEXT CLASSIFIER | [F1](#phase-f-shrink-the-model-to-clear-the-size-gate) and [F10](#phase-f-shrink-the-model-to-clear-the-size-gate) both measured the VISUAL TOWER only. The classifier was never in scope, and fp16 was chosen under size pressure rather than on evidence | ⬜ | **Open, and the current fp16 choice rests on a thin check.** The classifier is 16.36 of the 59.5 MiB download, so int8 takes it to about 8.2 MiB and the total to about 51 MiB. What exists today: fp32 is 32.7 MiB and breaks the 25 MiB per-file cap, so fp16 was picked, and it was verified at 100% top-1 and top-5 agreement with fp32 on **24 photos** with a row cosine cost of 1e-7. That is agreement on a tiny sample, not accuracy, and **int8 was never tried**. The classifier is a favourable quantisation target because rows are L2-normalised, so values are bounded and there are no activation outliers. Run it the way F10 was run: real top-1 over all 24,633 NABirds images for fp32, fp16 and int8, plus int8 with per-row scales, since a single global scale across 11,167 unrelated species embeddings is the obvious failure mode. Gate: keep 86.90. |
