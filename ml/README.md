...
| G2 | Export to ONNX | Any web runtime needs this. Core ML converts from torch directly and skips it | ✅ | Superseded by [G13](#phase-g-ship), which exports the shipping student bit-exact: worst cosine 1.00000000, max absolute difference 7.34e-07, 0/16 top-1 disagreements. **The fp16 blocker no longer matters.** fp16 export is still broken, because `convert_float_to_float16` emits Cast nodes whose declared output type does not match the consumer and onnxruntime rejects the graph at load, but fp16 was only needed for WebGPU and [G1](#phase-g-ship) selects WASM with int8. fp16 stays relevant only to Core ML, which converts from torch and never touches this path. |
...
| G16 | Add month to the occurrence prior | The prior is P(species\|cell) with no time dimension, so a Rufous Hummingbird in Seattle gets the same score in July and January | ⬜ | **Data exists, but a naive monthly split WILL lose accuracy. Measured.** `observed_on` is already a column in the iNat dump next to the lat/lon we use, and `build_occurrence.py` selects only `taxon_id, latitude, longitude`. This is a capability the Bayesian rewrite LOST: the old BirdLife path had `seasonalTrust` with breeding, nonbreeding and passage masks. ⚠️ **The corpus is big but the per-pair counts are not.** Over 26,396,703 (species, cell) pairs and 157,114,210 observations: median count is **1**, p90 is 10, **50.1% of pairs have exactly one observation**, and 91.3% have fewer than 12. Only **8.7%** could average even one observation per month. Those thin pairs still hold 35.7% of all observations, so this is not an edge case, and it hits RARE species hardest, which are the ones a geographic prior exists to rank correctly. The specific hazard is that the scorer treats a missing species as `OCC_FLOOR = log(1e-9)`, strong evidence of absence. That is fair annually and wrong monthly, where a zero usually means nobody birded that cell that month. So the build needs hierarchical backoff from (species, cell, month) toward (species, cell) when counts are thin, rather than a per-month swap. The `alpha` pseudo-count in `fit_occurrence.py` is the right hook and currently fits to about 1e-10 because it has nothing to smooth toward. Gate: beat 93.80 on the same split. |
