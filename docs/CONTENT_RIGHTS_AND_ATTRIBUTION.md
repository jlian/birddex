...
| OpenStreetMap (ODbL 1.0) | Local reverse geocoding archive | Web and iOS link `OpenStreetMap` to its copyright page below the location control; the route returns the full attribution string; the archive itself carries `attribution`, `license` and `license_url` in its PMTiles metadata per ODbL 4.2(b); the privacy policy offers the build method per ODbL 4.6(b) | Ready for the OSM-derived content |
| Wikipedia / Wikidata via Nominatim | Prominence scores joined into the private reverse-geocoding archive at build time | Nominatim explicitly publishes the preprocessed ranking table for installation and its documentation links the GPL-2.0 build source. The source values derive from Wikipedia links and Wikidata IDs (CC BY-SA 4.0 and CC0 respectively). WingDex does not distribute the table, archive, score, article text, or link graph: the private server-side score only orders OSM candidates and the API returns names without the score. The privacy policy names the source and purpose | Ready for the current server-only use. Reassess and carry the applicable data notices before making the archive or scores downloadable or returning scores through the API |
| Geoapify | Explicit place search only | Web and iOS show a linked `Geoapify` credit below the location control; privacy policy identifies Geoapify; WingDex does not cache provider responses | Ready |
