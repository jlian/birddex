...
| OpenStreetMap (ODbL 1.0) | Local reverse geocoding archive | Web and iOS link `OpenStreetMap` to its copyright page below the location control; the route returns the full attribution string; the archive itself carries `attribution`, `license` and `license_url` in its PMTiles metadata per ODbL 4.2(b); the privacy policy offers the build method per ODbL 4.6(b) | Ready for the OSM-derived content; see the Wikipedia/Wikidata row for the importance values joined into the same archive |
| Wikipedia / Wikidata via Nominatim | Prominence scores joined into the reverse geocoding archive at build time | Not separately surfaced in the UI: the value is a quantised 0-255 tie-breaker with no text or geometry from the source. The privacy policy names the source and its purpose; the build method is published | **Confirm** the redistribution terms for the derived importance values. Nominatim's `wikipedia-wikidata` scripts are GPL-2.0, but the values derive from Wikipedia and Wikidata dumps (CC BY-SA 4.0 and CC0 respectively), so the applicable terms for a derived numeric score need a decision before release |
