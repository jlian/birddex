...
| OpenStreetMap (ODbL 1.0) | Local reverse geocoding archive | Web and iOS link `OpenStreetMap` to its copyright page below the location control; the route returns the full attribution string; the archive itself carries `attribution`, `license` and `license_url` in its PMTiles metadata per ODbL 4.2(b); the privacy policy offers the build method per ODbL 4.6(b) | Ready |
