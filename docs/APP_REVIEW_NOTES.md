...
When **Use Location and Time** is enabled, WingDex uses photo location and month on the device to improve identification. Exact coordinates may be saved with the outing and photo metadata. Rounded coordinates may be sent through WingDex to Geoapify to suggest a location name. The location editor always shows linked **Powered by Geoapify · © OpenStreetMap contributors** attribution.
